...
async function check() {
    ...
    const { data: sales, error: err1 } = await supabase
        .from('sales')
        .select('id, invoice_number, hash, prev_hash, is_certified, created_at')
        .order('created_at', { ascending: false })
        .limit(3);

    if (err1) console.error('Error fetching sales:', err1.message);
    ...
    if (err2) console.error('Error fetching logs:', err2.message);
